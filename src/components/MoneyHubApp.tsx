          ...
          return (
            <div className="flex flex-col gap-5">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <button onClick={() => navigateTo('treasury')} className="text-left p-7 sm:p-8 rounded-[36px] border border-blue-500/25 bg-gradient-to-br from-blue-500/10 to-neutral-950 hover:border-blue-500/50 active:scale-[0.985] transition shadow-xl shadow-blue-950/10"><div className="flex justify-between items-start"><div className="h-11 w-11 rounded-2xl bg-blue-500/15 text-blue-300 flex items-center justify-center"><Coins className="h-5 w-5" /></div><span className="text-[9px] font-black text-blue-300 uppercase tracking-widest">Live · TND</span></div><p className="text-[11px] font-black text-neutral-400 uppercase tracking-[0.18em] mt-7">Caisse TND</p><p className={`text-4xl sm:text-5xl font-black tracking-[-0.07em] mt-2 ${metrics.tndBalance >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{formatRawCurrency(metrics.tndBalance || 0, 'TND')}</p><div className="flex flex-wrap gap-x-5 gap-y-1 mt-4 pt-4 border-t border-white/5 text-[10px] font-black"><span className="text-emerald-400">+ {formatRawCurrency(metrics.tndTodayIn || 0, 'TND')} aujourd’hui</span><span className="text-rose-400">− {formatRawCurrency(metrics.tndTodayOut || 0, 'TND')} aujourd’hui</span></div></button>
                <button onClick={() => navigateTo('currencies')} className="text-left p-7 sm:p-8 rounded-[36px] border border-emerald-500/25 bg-gradient-to-br from-emerald-500/10 to-neutral-950 hover:border-emerald-500/50 active:scale-[0.985] transition shadow-xl shadow-emerald-950/10"><div className="flex justify-between items-start"><div className="h-11 w-11 rounded-2xl bg-emerald-500/15 text-emerald-300 flex items-center justify-center"><WalletCards className="h-5 w-5" /></div><span className="text-[9px] font-black text-emerald-300 uppercase tracking-widest">Live · USD</span></div><p className="text-[11px] font-black text-neutral-400 uppercase tracking-[0.18em] mt-7">Position globale</p><p className={`text-4xl sm:text-5xl font-black tracking-[-0.07em] mt-2 ${metrics.netPosition >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{formatUSD(metrics.netPosition || 0)}</p><div className="flex flex-wrap gap-x-5 gap-y-1 mt-4 pt-4 border-t border-white/5 text-[10px] font-black"><span className="text-blue-300">{formatUSD(metrics.totalAvoirs || 0)} avoirs</span><span className="text-neutral-500">{activePartners} partenaire{activePartners > 1 ? 's' : ''} actif{activePartners > 1 ? 's' : ''}</span></div></button>
              </div>
